...
public class Heap {
    ...
    public void trickleUp(int idx) {
        ...
        while (idx > 0 && heapArray[parentIdex].getKey() < nodeToInsert.getKey()) {
            heapArray[idx] = heapArray[parentIdex]; // move parent down
            idx = parentIdex;
            parentIdex = (parentIdex - 1) / 2; // move up
        }
